...
public class CreateFolderlessListRequest {
	...
	public CreateFolderlessListRequest(String accessToken, Long spaceID, CUList list) {
		setAccessToken(accessToken);
		setSpaceID(spaceID);
		setList(list);
	}
	
	public ClickUpResponse<CUList> execute() {
		Gson gson = new Gson();
		HTTPRequest httpRequest = new HTTPRequest("https://api.clickup.com/api/v2/space/" + getSpaceID() + "/list", RequestMethod.POST);
		httpRequest.addHeaderProperty("Authorization", getAccessToken());
		httpRequest.addHeaderProperty("Content-Type", "application/json");
		httpRequest.addPayload(gson.toJson(getList()));
		
		Response response = httpRequest.executeRequest();
		
		return new ClickUpResponse<CUList>(CUList.class, response.getResponseCode(), response.getResponse(), ResponsesType.OBJECT);
	}
}
