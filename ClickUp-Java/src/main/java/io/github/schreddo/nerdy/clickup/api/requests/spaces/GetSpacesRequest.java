...
public class GetSpacesRequest {
	...
	public GetSpacesRequest(String accessToken, Long teamID, Boolean archived) {
	...
	public ClickUpResponse<BaseCollection> execute() {
		HTTPRequest httpRequest = new HTTPRequest("https://api.clickup.com/api/v2/team/" + getTeamID() + "/space", RequestMethod.GET);
		...
		return new ClickUpResponse<BaseCollection>(BaseCollection.class, response.getResponseCode(), response.getResponse(), ResponsesType.SPACES_BC);
	}
}
