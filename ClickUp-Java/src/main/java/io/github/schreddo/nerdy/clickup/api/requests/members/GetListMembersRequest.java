...
public class GetListMembersRequest {
	...
	public GetListMembersRequest(String accessToken, Long listID) {
		setAccessToken(accessToken);
		setListID(listID);
	}
	
	public ClickUpResponse<BaseCollection> execute() {
		HTTPRequest httpRequest = new HTTPRequest("https://api.clickup.com/api/v2/list/" + getListID() + "/member", RequestMethod.GET);
		...
		return new ClickUpResponse<BaseCollection>(BaseCollection.class, response.getResponseCode(), response.getResponse(), ResponsesType.MEMBERS_BC);
	}
}
