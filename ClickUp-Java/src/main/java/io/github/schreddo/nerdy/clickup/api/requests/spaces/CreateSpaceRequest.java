...
public class CreateSpaceRequest {
	...
	public CreateSpaceRequest(String accessToken, Long teamID, CUSpace space) {
		setAccessToken(accessToken);
		setTeamID(teamID);
		setSpace(space);
	}
	
	public ClickUpResponse<CUSpace> execute() {
		Gson gson = new Gson();
		HTTPRequest httpRequest = new HTTPRequest("https://api.clickup.com/api/v2/team/" + getTeamID() + "/space", RequestMethod.POST);
		httpRequest.addHeaderProperty("Authorization", getAccessToken());
		httpRequest.addHeaderProperty("Content-Type", "application/json");
		httpRequest.addPayload(gson.toJson(getSpace()));
		
		Response response = httpRequest.executeRequest();
		
		return new ClickUpResponse<CUSpace>(CUSpace.class, response.getResponseCode(), response.getResponse(), ResponsesType.OBJECT);
	}
}
